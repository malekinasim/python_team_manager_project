...
class TextFileStorage(Storage,Generic[T]):
    ...
    def create_instance(cls, config: dict,entity_class:T):
        ...
        cls.__storage_file_path=os.path.join(config.get('storage_file_dir'),f'data_{entity_class.__qualname__}.text')
        cls.__entity_class=entity_class
        return cls[entity_class]
    @classmethod    
    def find_by_id(cls, id)->T:
        lines=read_file_lines(cls.__storage_file_path)
        for line in lines:
            if( line.strip()!=''):
                entity=StringDecoder[T].decode(line, cls.__entity_class)
    ...
    def save_entity(cls, data:T):
        if not os.path.exists(cls.__storage_file_path):
            write_file(cls.__storage_file_path,data.code_entity())
        else:
            append_file(cls.__storage_file_path,data.code_entity())
    ...
    def update_entity(cls, data:T):
        ...
        if(updated):  
            write_file(cls.__storage_file_path,'\n'.join(lines))
        else:
