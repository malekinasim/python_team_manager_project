...
def append_to_json_file(file_path,data):
    ...
    try:
        ...
        json.dump(json_array,file,indent=4) #convert json to string and write to file by tab space
        file.truncate() # Remove any remaining data after the new JSON data has been written
    except OSError as e:
        raise ValueError(f"Error opening file {file_path}: {e}")
    finally:
        if file: 
