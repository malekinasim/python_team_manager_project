...
from controllers.basic_manager import BasicManager
...
class TeamManager(BasicManager[Team]):
    ...
    def find_all_by_team_type(self,team_type):
        return super().find_all_by_properties({
    ...
    def update_team(self, entity:Team,name, type, fee_amount=0,membership_fee=0):
        entity.set_name(name)
        entity.set_type(type)
        entity.set_fee_amount(fee_amount)
        if(fee_amount==membership_fee):  
                entity.set_fee_paid(True)
        elif(fee_amount>0):
            entity.set_fee_paid(False) #uncompleted payment have debt
        else:
            entity.set_fee_paid(False)
        return super().update_entity(entity)
    
    def add_team(self, name, type, fee_amount=0,membership_fee=0):
        fee_paid=False
        if(fee_amount!=0):
            if(fee_amount>=membership_fee):  
                fee_paid=True
        ...
        id=0
        try:
            item_list=super().find_all()
            max_ids=0
            if(len(item_list)>0):
                max_ids=max([item.get_id() for item in item_list])
            id=max_ids+1
            code=f"TEAM-{id}"
